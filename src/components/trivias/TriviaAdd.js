...
class TriviaAdd extends Component {
  constructor(props) {
    super(props);
    this.state = {
      triviaQuestions: data.trivias[0].questions,
      question: data.trivias[0].questions[0],
      checked: '',
      prevChecked: '',
      checkedAnswer: '',
      questionsCorrect: 0,
  ...
  nextQuestion = () => {
    ...
    this.setState({
      question: data.trivias[0].questions[newIndex]
    })
  }
  ...
  answerSubmit(e){
    ...
    if(this.state.checkedAnswer){
      ...
      if(this.state.question.index < this.state.triviaQuestions.length-1){
        const newIndex = this.state.question.index + 1;
        // reset checkedAnswer so submit only works if answer selected on subsequent questions.
        this.setState({
          question: data.trivias[0].questions[newIndex],
          checked: '',
          prevChecked: '',
